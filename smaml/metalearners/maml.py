...
class ModelAgnosticMetaLearning(object):
    ...
    def __init__(self, model, optimizer=None, step_size=0.1, first_order=False,
                 ...
                 loss_function=kernel_loss, device=None):
    ...
    def get_outer_loss(self, batch):
        ...
        if is_classification_task:
            results.update({
                ...
                'grad_norm': np.zeros((num_tasks,), dtype=np.float32)
            })
        ...
        for task_id, (train_inputs, train_targets, test_inputs, test_targets) \
            ...
            if is_classification_task:
                ...
                # print(results['grad_norm'])
                results['grad_norm'][task_id] = adaptation_results['grad_norm'][0]
    ...
    """
    inner loop parameter update rule
    batch에 대한 update
    """
    def adapt(self, inputs, targets, is_classification_task=None,
        ...
        results = {
            ...
            'grad_norm': np.zeros((num_adaptation_steps,), dtype=np.float32)
            }

        for step in range(num_adaptation_steps):
            logits = self.model(inputs, params=params)
            inner_loss = self.loss_function(logits, targets)
            grad_norm = torch.norm(torch.autograd.grad(inner_loss, self.model.parameters(), create_graph=True)[0])
            results['inner_losses'][step] = inner_loss.item()
            results['grad_norm'][step] = grad_norm
            ...
            params = stiefelmanifold_update_parameters(self.model, inner_loss,
    ...
    def evaluate(self, dataloader, max_batches=500, verbose=True, **kwargs):
        ...
        with tqdm(total=max_batches, disable=not verbose, **kwargs) as pbar:
            for results in self.evaluate_iter(dataloader, max_batches=max_batches):
                ...
                #     grad_norm_list.extend(results['grad_norm'])
                pbar.set_postfix(**postfix)
        ...
        # if grad_norm_list:
        #     batch_result = np.mean(grad_norm_list)
        # else:
        #     batch_result = None

        return mean_results, result_list
