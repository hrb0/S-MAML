...
from maml.model import ModelConvOmniglot, ModelConvMiniImagenet, ModelMLPSinusoid, ModelConvFC100, ModelConvCUB
...
def get_benchmark_by_name(name,
    ...
    if name == 'sinusoid':
        ...
        meta_train_dataset = FC100(folder,
                                   transform=transform,
                                   target_transform=Categorical(num_ways),
                                   num_classes_per_task=num_ways,
                                   meta_train=True,
                                   dataset_transform=dataset_transform,
                                   download=True)
        meta_val_dataset = FC100(folder,
                                 transform=transform,
                                 target_transform=Categorical(num_ways),
                                 num_classes_per_task=num_ways,
                                 meta_val=True,
                                 dataset_transform=dataset_transform)
        meta_test_dataset = FC100(folder,
                                  transform=transform,
                                  target_transform=Categorical(num_ways),
                                  num_classes_per_task=num_ways,
                                  meta_test=True,
        ...
        model = ModelConvFC100(num_ways, hidden_size=hidden_size)
        loss_function = F.cross_entropy
        ...
        transform = Compose([Resize(252), CenterCrop(84), ToTensor()])
        ...
        model = ModelConvCUB(num_ways, hidden_size=hidden_size)
        loss_function = F.cross_entropy

    elif name == 'm2c':
        transform_m = Compose([Resize(84), ToTensor()])
        transform_c = Compose([Resize(252), CenterCrop(84), ToTensor()])
